const countryContent = {
  ...
  function openPopup(countryCode) {
    ...
    if (!data) {
      console.warn(`Country code "${countryCode}" not found in countryContent`);
      return;
    }
  
    // Check if required DOM elements exist
    const popup = document.getElementById("countryPopup");
    const countryName = document.getElementById("popupCountryName");
    const flag = document.getElementById("popupFlag");
    const disclaimerText = document.getElementById("popupDisclaimerText");
    const linksContainer = document.getElementById("popupLinks");
    
    if (!popup || !countryName || !flag || !disclaimerText || !linksContainer) {
      console.error("Required popup elements not found in DOM");
      return;
    }
    ...
    disclaimerText.innerHTML = data.text;
    ...
    data.links.forEach(link => {
      ...
      // Open in same window for better UX and to avoid popup blockers
      // Add security attributes only if external link
